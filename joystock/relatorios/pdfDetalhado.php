<?php
	...
	$where .= !empty($_GET['COD_PRD'])?" AND T2.COD_PRD = {$_GET['COD_PRD']} ":"";
	...
	$query = "SELECT T1.COD_PRD, T1.COD_MVM, T1.HST_PRD, T3.DTA_MVM, T2.NME_PRD, NME_CLN, T1.QTD_PRD AS QTD, T1.VLR_TPR AS VLR FROM ".
			 ...
			 "WHERE T2.EXC_UPCSPRD IS NULL AND T1.TPO_MVM = '$tipo' $where ORDER BY T1.COD_PRD DESC";
    ...
    $pdf->Cell(277,10,utf8_decode("Relatório Detalhado de {$tp[$tipo]}"),0,1,'C',0,'');
    ...
    $pdf->Cell(68,5,utf8_decode('PRODUTO'),1,0,'C',1,'');
    $pdf->Cell(40,5,utf8_decode('HISTÓRICO'),1,0,'C',1,'');
    $pdf->Cell(20,5,utf8_decode('DATA'),1,0,'C',1,'');
	$pdf->Cell(69,5,utf8_decode('CLIENTE'),1,0,'C',1,'');    
	...
	while ($row = @mysql_fetch_array($dados)){
    	...
    	$pdf->Cell(68,5,$row['NME_PRD'],1,0,'L',0,'');
    	$pdf->Cell(40,5,$row['HST_PRD'],1,0,'C',0,'');
		$pdf->Cell(20,5,$BDFB->MostrarCampo("date",$row['DTA_MVM']),1,0,'C',0,'');
		$pdf->Cell(69,5,$row['NME_CLN'],1,0,'L',0,'');
