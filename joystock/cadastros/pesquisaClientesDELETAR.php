<?
	...
	$query = "SELECT * FROM upcscln_tb T1 ".
			 "WHERE T1.EXC_UPCSCLN IS NULL AND ISS_CLN = '1' $where ORDER BY COD_CLN DESC LIMIT {$pag->getLimit()},20";
					...
					<div id="mLogin" style="height:100%; width:100%; overflow-y:auto; overflow-x:hidden">
						<table width="100%" height="97%" cellpadding="0" cellspacing="0">
							<tr  style="position:relative;  " height="30px" width="100%" >
								<td valign="top" id="letra" style="font-size:30px;  " colspan="12" align="center">Pesquisa de Clientes</td>
								<div style="position:absolute;right:5px; padding:2px ">
									<img id="box1" title="Atualizar"    onclick="location.href='/joystock/cadastros/pesquisaClientes.php'" style="width:28px; cursor:pointer; height:28px;" src="/joystock/pcte-common/images/actualiser.png">
									<img id="box1" title="Voltar"    onclick="location.href='/joystock/cadastros/clientes.php'" style="width:28px; cursor:pointer; height:28px;" src="/joystock/pcte-common/images/back.png">
								</div>
							</tr>
							<tr>
								<td>
							...
							<?
								...
								while ($row = @mysql_fetch_array($dados)){
									...
									$out .= '<tr onclick="window.location.href = \'/joystock/cadastros/clientes.php?COD='.$row['COD_CLN'].'\';" bgcolor="'.$bgcolor.'" style="color:'.$color2.'; cursor:pointer;  font-size:11px" >'.
