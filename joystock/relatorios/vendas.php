<?	
	...
	$form = "vendas.php?TIPO=".$_GET['TIPO'];
				...
				<div style="height:-webkit-calc(100% - 95px); width:100%; overflow-y:auto; overflow-x:hidden">
					<table width="100%" height="100%" cellpadding="0" cellspacing="0">
						<tr  style="position:relative;  height:45px " width="100%" >
							<td valign="top" id="letra" style="border-bottom:solid 2px #333; font-size:30px; padding-top:5px; " colspan="12" align="center">Relatório Agregado de <?=$tp[$_GET['TIPO']]?></td>
							<div style="position:absolute;right:5px; top:5px; padding:2px ">
								<img id="box1" title="Atualizar"    onclick="location.href='/joystock/relatorios/vendas.php?TIPO=<?=$_GET['TIPO']?>'" style="width:28px; cursor:pointer; height:28px;" src="/joystock/pcte-common/images/actualiser.png">
						...
						<tr style="height:100%">
							<td align="center">
								<iframe scrolling="No" src="pdfVendas.php?COD_PRD=<?=$_POST['COD_PRD']?>&COD_CLN=<?=$_POST['COD_CLN']?>&NME_CLN=<?=$_POST['NME_CLN']?>&COD_GCL=<?=$_POST['COD_GCL']?>&DTA_VN1=<?=$_POST['DTA_VN1']?>&DTA_VN2=<?=$_POST['DTA_VN2']?>&TIPO=<?=$_GET['TIPO']?>" name="FrmPrincipal" frameborder="0" border="0" height="100%" width="100%" ></iframe>
							</td>
						</tr>
						<tr >
							<td align="center">
